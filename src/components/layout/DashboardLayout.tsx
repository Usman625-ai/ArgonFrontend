...
export default function DashboardLayout({ title, subtitle }: P) {
 ...
 // Lock body scroll when mobile sidebar or user menu is open
 useEffect(() => {
   if (ms || um) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Sidebar
        ...
        mobileOpen={ms}
        onClose={() => setMs(false)}
        onToggleCollapse={() => setSc(!sc)}
      />

      <div
        className={cn(
          'flex flex-col transition-all duration-300',
          sc ? 'lg:pl-20' : 'lg:pl-64'
        )}
        onClick={() => {
          if (ms) setMs(false);
        }}
      >
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border bg-background/85 px-4 backdrop-blur-md lg:px-7">
          <div className="flex items-center gap-4">
            ...
            <div>
              {title && (
                <h1 className="text-[1.15rem] font-semibold tracking-tight">{title}</h1>
              )}
              {subtitle && (
          ...
          <div className="flex items-center gap-1.5">
            ...
            <NotificationBell
              basePath={user?.role === 'ADMIN' ? '/api/admin' : '/api/seller'}
            />
            ...
            <div className="relative">
              <button
                onClick={() => setUm(!um)}
                className="flex items-center gap-2.5 rounded-lg py-1 pl-1 pr-2 hover:bg-accent transition-colors"
              >
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-primary-400 to-primary-600 text-[0.8rem] font-semibold text-primary-foreground">
                  {getInitials(user?.name || 'U')}
                </div>
                <div className="hidden text-left sm:block">
                  <p className="text-sm font-medium leading-tight">{user?.name || 'User'}</p>
                  <p className="text-[0.7rem] leading-tight text-muted-foreground capitalize">
              ...
              <AnimatePresence>
                {um && (
                  <motion.div
                    ...
                    className="absolute right-0 top-12 w-52 overflow-hidden rounded-xl border border-border bg-card shadow-xl"
                  >
        ...
        <main className="flex-1 p-4 lg:p-7">
          <motion.div
            key={loc.pathname} // ← ADDED: re-triggers animation on every page change
