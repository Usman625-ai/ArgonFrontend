...
export default function Sidebar({ collapsed, mobileOpen, onClose, onToggleCollapse }: P) {
  ...
  return (
    <>
      {/* Mobile backdrop — plain div, NO AnimatePresence/motion */}
      {mobileOpen && (
        <div
          onClick={onClose}
      ...
      <aside
        className={cn(
          'fixed left-0 top-0 z-40 flex h-screen flex-col bg-[#15110d] transition-transform duration-300 border-r border-white/[0.06]',
          collapsed ? 'w-20' : 'w-64',
      ...
      >
        <div className="flex h-16 shrink-0 items-center justify-between px-4">
            ...
            {!collapsed && (
              <div className="leading-tight">
                ...
                <span className="flex items-center gap-1 text-[0.65rem] font-medium text-primary-300">
                  ...
                  {user?.role === 'ADMIN' ? 'Administration' : 'Seller Console'}
                </span>
              </div>
            )}
          </div>
          {mobileOpen && (
            <button
              onClick={onClose}
              className="rounded-lg p-1 text-[#f0e9dd] hover:bg-white/5 lg:hidden"
            >
              <X className="h-5 w-5" />
            </button>
          )}
        ...
        <nav className="flex-1 space-y-5 overflow-y-auto scrollbar-thin px-3 py-3">
          {groups.map((group) => (
            <div key={group.label}>
              {!collapsed && (
                <p className="mb-1.5 px-3 text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-white/30">
                  {group.label}
                </p>
              )}
                    ...
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        cn(
                          ...
                          isActive
                            ? 'bg-white/[0.07] text-primary-300'
                    ...
                    >
                      {({ isActive }) => (
                        <>
                          {isActive && (
                            <motion.span
                              layoutId={`active-pill-${user?.role}`}
                              className="absolute left-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-r-full bg-primary-400"
                            />
                          )}
        ...
        <div className="border-t border-white/[0.06] p-3">
          <button
            ...
            className={cn(
              'flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-white/50 transition-colors hover:bg-destructive/10 hover:text-destructive',
              collapsed && 'justify-center'
            )}
          >
          ...
          {onToggleCollapse && (
            <button
              ...
              className="mt-1 hidden w-full items-center justify-center rounded-lg p-2 text-white/40 hover:bg-white/5 hover:text-white/70 lg:flex"
            >
              <ChevronLeft
